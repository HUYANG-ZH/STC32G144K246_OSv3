#ifndef SHARED_POS_PID_H
#define SHARED_POS_PID_H

#include "zf_common_typedef.h"

typedef enum
{
    SHARED_POS_PID_DIRECTION_DIRECT = 0,                                  // 正作用 输出增大时反馈增大
    SHARED_POS_PID_DIRECTION_REVERSE,                                     // 反作用 输出增大时反馈减小
} shared_pos_pid_direction_enum;

typedef enum
{
    SHARED_POS_PID_DERIVATIVE_ON_FEEDBACK = 0,                            // 对反馈微分 减少目标突变带来的 D 项冲击
    SHARED_POS_PID_DERIVATIVE_ON_ERROR,                                   // 对误差微分
} shared_pos_pid_derivative_enum;
...
typedef struct
{
    ...
    uint8 integral_limit_enable;                                          // 积分项限幅使能 ZF_ENABLE/ZF_DISABLE
    float integral_min;                                                   // 积分项最小值
    float integral_max;                                                   // 积分项最大值

    uint8 delta_limit_enable;                                             // 输出变化量限幅使能 ZF_ENABLE/ZF_DISABLE
    float delta_min;                                                      // 输出变化量最小值
    float delta_max;                                                      // 输出变化量最大值
    ...
    shared_pos_pid_direction_enum direction;                              // 控制方向
    shared_pos_pid_derivative_enum derivative_mode;                       // 微分模式

    float output;                                                         // 当前输出
    float ramp_target;                                                    // 斜坡处理后的目标值
    float integral;                                                       // 积分项输出值
