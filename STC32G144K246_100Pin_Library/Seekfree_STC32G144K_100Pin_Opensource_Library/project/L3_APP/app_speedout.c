...
static void app_speedout_tick(void)
{
    ...
    /* 堵转保护(与car2同步): 使用上一帧已发布数据判定, 触发时投递的急停邮箱
    ...
    /* 速度环前馈(自car3移植): 目标速度直接折算 PWM 叠加到闭环输出。
       符号: 速度环 PID 为 REVERSE 方向(正目标=前进=>负PWM), 前馈取 -target*KFF
       与反馈同向, 避免前馈与反馈方向相反; 浮点转整形用 TFPU。
       极性核对: car4 与 car3 的 PID方向(REVERSE)/目标符号(+1)/电机FORWARD_LEVEL(GPIO_LOW)/
       编码器符号 均一致, 故符号逻辑直接沿用。 */
    {
        int32 ff_left;
        int32 ff_right;
