...
/*
 * All host/APP/Bootloader communication uses the nominal baud rate 2,000,000.
 *
 * Cold entry: the chip runs at 48 MHz.
 * Timer1 divisor 6 gives an exact 2,000,000 baud and IAP_TPS must be 48.
 *
 ...
 * the SEEKFREE UART driver selects divisor 15 for a requested 2,000,000 baud,
 * producing about 2,066,667 baud. The PC still opens the port at 2,000,000;
 * this is the same configuration the user has verified as stable. IAP_TPS
 * must remain 124 for Flash programming at the retained clock.
 *
...
/* v2.6: turbo incremental page CRC, fast program/erase and 2 Mbaud UART. */
